...
Product.prototype.update = function () {
	...
	return new Promise(function (resolve, reject) {
		connection.getConnection(function (error, connection) {
			...
			let values = [that.name, that.companyId, that.description, that.unit, that.hsnCode, that.imgName, that.isActive, that.createdBy, that.id];

			connection.query("UPDATE product SET name = ?,companyId = ?,description = ?,unit = ?,hsnCode = ?,imgName = ?,isActive = ?, updatedBy = ? WHERE id = ?", values, function (error, rows, fields) {
				if (!error) {
					resolve(rows);
				} else {
					console.log("Error...", error);
...
Product.prototype.allByCompanyId = function (companyId) {
	return new Promise(function (resolve, reject) {
		connection.getConnection(function (error, connection) {
			...
			connection.query('select p.id, p.name, p.description, p.unit, p.hsnCode, p.imgName, p.dateTimeCreated, u.name as createdBy, p.isActive from product p inner join user u on p.createdBy = u.id where p.companyId=? and p.isActive = 1', [companyId, isActive], function (error, rows, fields) {
